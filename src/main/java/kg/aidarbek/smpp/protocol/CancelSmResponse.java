package kg.aidarbek.smpp.protocol;

import java.util.Objects;

/**
 * Immutable cancellation response; the command status is carried by its PDU envelope.
 * @param optionalParameters ordered raw extensions, including supported 5.0 congestion on success
 */
public record CancelSmResponse(OptionalParameters optionalParameters) implements Command {
    /** Requires non-null immutable parameters. */
    public CancelSmResponse {
        Objects.requireNonNull(optionalParameters, "optionalParameters");
    }

    @Override
    public long commandId() {
        return 0x80000008L;
    }

    @Override
    public String toString() {
        return "CancelSmResponse[optionalParameterCount="
                + optionalParameters.entries().size() + "]";
    }
}
